...
public class UsersPage extends JPanel {
    ...
    private List<User> allItems       = new ArrayList<>();
    private List<User> displayedItems = new ArrayList<>();
    ...
    private JPanel buildContent() {
        ...
        JLabel sub = new JLabel("Manage system users and access levels");
    ...
    private JPanel buildCard() {
        ...
        JPanel toolbar = new JPanel(new MigLayout("insets 0, fillx", "[grow][90!][90!][90!]", "[40!]"));
        toolbar.setOpaque(false);
        searchField = TaxPage.styledField("Search users...");
        searchField.getDocument().addDocumentListener(new DocumentListener() {
            public void insertUpdate(DocumentEvent e)  { filter(); }
            public void removeUpdate(DocumentEvent e)  { filter(); }
            public void changedUpdate(DocumentEvent e) { filter(); }
        });
        var editBtn    = TaxPage.btn("Edit Role",  UIConstants.INFO_COLOR);
        ...
        tableModel = new DefaultTableModel(
            new Object[]{"ID", "Full Name", "Username", "Email", "Role", "Created"}, 0) {
        ...
        table.getColumnModel().getColumn(4).setCellRenderer(roleRenderer);
        table.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseClicked(java.awt.event.MouseEvent e) {
                if (e.getClickCount() == 2) openEdit();
            }
        });
        int[] widths = {60, 200, 160, 220, 110, 120};
    ...
    private void openEdit() {
        User sel = selected();
        if (sel == null) { setStatus("Select a user to edit", UIConstants.WARNING_COLOR); return; }
        if (!svcOk()) return;

        JDialog dlg = TaxPage.styledDialog(shell, "Edit User Role", 420, 260);
        JPanel form = TaxPage.dialogForm();

        JTextField nameField = TaxPage.styledField("");
        nameField.setText(sel.getFullName());
        nameField.setEditable(false);
        nameField.setFocusable(false);
        nameField.setForeground(UIConstants.TEXT_MUTED);
        ...
        rl.setFont(UIConstants.FONT_LABEL);
        rl.setForeground(UIConstants.TEXT_SECONDARY);
        form.add(rl, "aligny center, gapy 8 0");
        form.add(roleBox, "h 40!, growx, wrap");
    ...
    private void deleteSelected() {
        User sel = selected();
        if (sel == null) { setStatus("Select a user to delete", UIConstants.WARNING_COLOR); return; }
    ...
    private void filter() {
        ...
        List<User> src = term.isEmpty() ? allItems : allItems.stream()
            .filter(u -> contains(u.getFullName(), term) || contains(u.getUsername(), term)
                      || contains(u.getEmail(), term) || contains(u.getRole(), term))
            .toList();
        displayedItems = new ArrayList<>(src);
        tableModel.setRowCount(0);
        for (User u : src)
            tableModel.addRow(new Object[]{u.getUserId(), u.getFullName(), u.getUsername(),
                u.getEmail(), u.getRole(), u.getCreatedAt()});
        setStatus(src.size() + " user(s)", UIConstants.TEXT_SECONDARY);
    }
